...
def choose(paragraphs, select, k):
    ...
    for i in range(len(paragraphs)):
        if select(paragraphs[i]):
           select_after += [paragraphs[i]] 
    if k >= len(select_after):
        return ''
    return select_after[k]
    # END PROBLEM 1
...
def accuracy(typed, reference):
    ...
    for i in range(min( len(typed_words), len(reference_words))):
        if typed_words[i] == reference_words[i]:
            acc_sub += 1
    acc = round( ( acc_sub / len(typed_words) )*100, 2)
    return acc
    # END PROBLEM 3
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for i in range(len(valid_words)):
        diff_func = diff_function(user_word, valid_words[i], limit)
        if diff_func <= limit and diff_func < min_diff:  
            min_diff = diff_func
...
def shifty_shifts(start, goal, limit):
    ...
    """
    # BEGIN PROBLEM 6
    result, fact, real = 0, 0, 0
    if len(start) != len(goal):
        result = abs(len(start) - len(goal))
    def shift(start, goal, real):
        nonlocal fact
        if (result + real) > limit:
            return limit + 1
        if not start or not goal:
            return result
        if start[0] != goal[0]:
            real += 1
            fact = 1 + shift(start[1:], goal[1:], real)
            return fact
        fact = shift(start[1:], goal[1:], real)
        return fact
    return shift(start, goal, 0)
    # END PROBLEM 6
...
def time_per_word(times_per_player, words):
    ...
    for i in range(len(times_per_player)):
        for k in range(1,len(times_per_player[i])):
            times[i] += [times_per_player[i][k] - times_per_player[i][k-1]]
            k += 1
...
def fastest_words(game):
    ...
    for i in word_indices:
        ...
        for k in player_indices:
            if time(game, k, i) <= min:
                min = time(game, k, i)
        for index in player_indices:
            if time(game, index, i) == min:
                break
        fastest_players[index] += [word_at(game, i)]
